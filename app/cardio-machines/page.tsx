...
export default function CardioMachinesPage() {
  ...
  const content = translations[locale].pageContent.cardioMachines;

  return (
    <>
      <section className="relative w-full h-screen min-h-[500px] overflow-hidden flex items-center justify-center">
        <img src="https://farisgroup.net/wp-content/uploads/2023/05/5QV4TcUWvsfYmm9hAdWXHD.jpg" alt={content.heroTitle} className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="container-custom">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="rounded-2xl overflow-hidden shadow-xl">
              <img src="https://farisgroup.net/wp-content/uploads/2023/05/5QV4TcUWvsfYmm9hAdWXHD.jpg" alt={content.title} className="w-full h-80 object-cover" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {projectImages.map((src, index) => (
              <div key={index} className="overflow-hidden rounded-xl shadow-md aspect-square">
                <img src={src} alt="Cardio machines project" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
              </div>
            ))}
          </div>
        </div>
