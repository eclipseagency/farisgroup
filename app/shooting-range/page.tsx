...
export default function ShootingRangePage() {
  ...
  const content = translations[locale].pageContent.shootingRange;
        ...
        <div className="container-custom">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center mb-16">
            <div className="rounded-2xl overflow-hidden shadow-xl">
              <img src="https://farisgroup.net/wp-content/uploads/2023/05/Rangers-Range-3-lanes-indoor-target-shooting-project-1-2.jpeg" alt="{content.title}" className="w-full h-80 object-cover" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {projectImages.map((src, index) => (
              <div key={index} className="overflow-hidden rounded-xl shadow-md aspect-square">
                <img src={src} alt="Shooting range project" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
              </div>
            ))}
          </div>
        </div>
